...
from utilities.nn.cnn import MiniVGGNet_9
...
(data, labels) = sdl.load(imagePaths, verbose=24)
...
# ADAM optimizer
# SGD
optimizer = SGD(lr=0.01, decay=0.01/35, momentum=0.8, nesterov=True)
model = MiniVGGNet_9.build(width=32, height=32, depth=3, classes=len(classNames))
model.compile(loss="categorical_crossentropy", optimizer=optimizer, metrics=["accuracy"])

# Train the network
print("[INFO]: Training....")
H = model.fit_generator(aug.flow(train_x, train_y, batch_size=24), validation_data=(test_x, test_y), class_weight=class_weight, steps_per_epoch=len(train_x) // 24, epochs=35, verbose=1)
#H = model.fit(train_x, train_y, validation_data=(test_x, test_y), class_weight=class_weight, batch_size=24, epochs=35, verbose=1)
...
plt.plot(np.arange(0, 35), H.history["acc"], label="train_acc")
...
# $ python minivggnet_covid19_CT_9.py --dataset covid19_CT_images --model covid19_CT_weights_9.hdf5