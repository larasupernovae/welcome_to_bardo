...
from utilities.nn.cnn import MiniVGGNet_2
...
optimizer = SGD(lr=0.01, decay = 0.01/100)
model = MiniVGGNet_2.build(width=32, height=32, depth=3, classes=len(classNames))
model.compile(loss="binary_crossentropy", optimizer=optimizer, metrics=["accuracy"])

# Train the network
print("[INFO]: Training....")
H = model.fit_generator(aug.flow(train_x, train_y, batch_size=32), validation_data=(test_x, test_y), class_weight=class_weight, steps_per_epoch=len(train_x) // 32, epochs=100, verbose=1)
...
plt.title("Training Loss and Accuracy on covid minivggnet")
plt.xlabel("Epoch #")
...
# $ python minivggnet_covid19_CT_2.py --dataset covid19_CT_images --model covid19_CT_2_weights.hdf5